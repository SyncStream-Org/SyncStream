      ...
      if (res.ok) {
        ...
        if (response[1] === 'true') {
          window.electron.ipcRenderer.invokeFunction('show-message-box', {
            ...
            message:
              'Your account is using an autogenerated password. Please change!',
          });
        }

        SessionState.getInstance().sessionToken = response[0];
...
export const getRoomPresence = (): Promise<{
  success: SuccessState;
  data?: Types.RoomPresenceData[];
}> => {
  const headers: Headers = generateDefaultHeaders();
  ...
  return fetch(request)
    .then(async (res) => {
      if (res.status === 200) {
        ...
        return {
          success: SuccessState.SUCCESS,
          data: body as Types.RoomPresenceData[],
        };
      }

      printUnexpectedError('user/presence GET API Call Failed', res);
      return { success: SuccessState.ERROR };
    })
    .catch((error) => {
      console.error(`Fetch Encountered an Error:\n${error}`);
